...
- Test phone: `96598765432` (Kuwait)
- Admin: `admin` / `password`
- Test user: `testuser` / `Test@12345` (subscriber, phone `96598765432`)
...
| Valid | `YOUR_API_USERNAME` / `YOUR_API_PASSWORD` | "✓ Connected as YOUR_API_USERNAME", Sender ID populated | Pass |
...
**Setup:** Register new user `otp_testuser` at `/wp-login.php?action=register` with phone `96598765432`
...
**Debug log entry:**
```
[send_sms()] type=welcome phone=96598765432 sender=KWT-SMS
...
**Setup:** OTP mode = 2FA, testuser (subscriber) has phone `96598765432`, Subscriber role requires OTP
...
**SMS log:** `login / 96598765432 / Sent / OK`
...
2. Enter local number `98765432` → "Send OTP Code"
...
**SMS log:** `passwordless / 96598765432 / Sent / OK`
...
| Issue | Detail | Status |
|---|---|---|
| Double country code on profile save | Admin entering `96598765432` in the "Local number" field on user-edit.php causes JS to prepend `965` again → stored as `96596598765432` | Fixed in v2.8.0 |
