...
class KwtSMS_Captcha {
	...
	private function verify_recaptcha_v3( $token ) {
		if ( empty( $token ) ) {
			return new WP_Error(
				...
				__( 'CAPTCHA verification failed. Please try again.', 'wp-kwtsms-otp' )
			);
		}
		...
		if ( isset( $data['score'] ) && $data['score'] < 0.5 ) {
			return new WP_Error(
				...
				__( 'Your request was flagged as suspicious. Please try again.', 'wp-kwtsms-otp' )
			);
		}
	...
	private function verify_turnstile( $token ) {
		if ( empty( $token ) ) {
			return new WP_Error(
				'kwtsms_captcha_missing',
				__( 'Please complete the security check before continuing.', 'wp-kwtsms-otp' )
			);
		}
		...
		if ( ! isset( $data['success'] ) || ! $data['success'] ) {
			return new WP_Error(
				'kwtsms_captcha_failed',
				__( 'Security check failed. Please try again.', 'wp-kwtsms-otp' )
			);
		}
